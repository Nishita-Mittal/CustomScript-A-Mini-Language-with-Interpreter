...
def p_statement_assignment(p):
    'statement_assignment : ID EQUALS expression'
    p[0] = {'type': 'assignment', 'name': p[1], 'value': p[3]}

# Assignment statement with semicolon
def p_statement_assignment_stmt(p):
    'statement : statement_assignment SEMICOLON'
    p[0] = p[1]
...
def p_statement_for(p):
    'statement : FOR LPAREN for_init SEMICOLON expression SEMICOLON for_update RPAREN LBRACE statement_list RBRACE'
    p[0] = {
        'type': 'for',
        'init': p[3],
        'condition': p[5],
...
def p_for_init(p):
    '''for_init : VAR ID EQUALS expression
...
def p_for_update(p):
    '''for_update : ID EQUALS expression
                  | empty'''
    if len(p) == 4:
        p[0] = {'type': 'assignment', 'name': p[1], 'value': p[3]}
    else:
...
# (for extra semicolons or empty blocks)
